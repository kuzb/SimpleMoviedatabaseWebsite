...
if($name == ""){
    echo "<div>No Actor Found</div><br>";
}
else {
$sql = "SELECT movie.title, movie.releasedate, movie.rating
            FROM movie, actor,acts, person
            WHERE movie.movieid = acts.movieid
            AND acts.actorid = actor.actorid
            AND actor.actorid = person.personid
...
if($count==0) {
    echo "<div>No Actor Found with the Name and Surname: $name $surname</div>";
}
else { echo "<h1>Movies $name $surname acted in</h1>"
?>
