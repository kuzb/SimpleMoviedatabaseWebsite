 ...
 * User: Burak KUZ
 * Date: 20.03.2017
 * Time: 20:18 'orderInput'
 */

$db = mysqli_connect('localhost', 'root', '', 'movies');
$orderType = $_POST['orderInput'];

$sql = "SELECT m.title, p.personname, p.surname, m.releasedate, m.rating
        FROM directs d, movie m, person p 
        WHERE p.personid = d.directorid AND d.movieid = m.movieid
        ORDER BY $orderType";
...
<?php
echo "<h1>Movies are ordered by $orderType</h1>"
?>
