...
if($genrename == ""){
    echo "<div>No Moive found for given genre</div><br>";
}
else {

//Prepare the query
$sql = "SELECT movie.title, movie.releasedate, movie.rating
            FROM award, awardtomovie, movie
            WHERE movie.movieid = awardtomovie.movieid
            AND awardtomovie.awardid = award.awardid
            AND award.awardname ='$genrename'";
...
if($count==0) {
    echo "<div>No movie found that won: $genrename</div>";
}
else {
echo "<h1>Movies that won $genrename</h1>"
?>
